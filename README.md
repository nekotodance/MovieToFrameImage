...
> - 最終フレームは少しブレているので少しだけ手前のフレームから続きを作りたい  
> - 前半がイマイチなので途中のフレームをエンドフレームとした動画を生成し直したい  
> - 次の動画を作る際のイケてるフレーム手早く探したい  
...
- コピー先のディレクトリ設定
  - image-fcopy-dir   : R,Returnキーによるファイルのコピー先フォルダ名  
> [!CAUTION]
...
- ウインドウタイトル  
    例）\[1/4\], ( 6/49), filename  
    \[ファイル番号/ファイル総数\], (フレーム番号/フレーム総数) ファイル名 横x縦  
    左から順番に
    - 表示中のファイル番号/同じ階層の動画ファイル総数  
    - 表示中のフレーム番号/フレーム総数  
    - ファイル名
    - 横x縦のピクセル数
    を表示します
...
- ステータス部分  
    ...
    \[File:ファイル番号/ファイル総数\], (Frame:フレーム番号/フレーム総数), ロード状態, 再生状態  
    左から順番に
    - File：表示中のファイル番号/同じ階層の動画ファイル総数  
    - Frame：表示中のフレーム番号/フレーム総数  
    - ロード済みフレーム数/フレーム総数、もしくはLoaded（ロード完了）  
    - 再生状態 stop, play, pauseの後ろに再生速度（1x - 8x）  
    もしくはファイルのロード状態やエラー状態などを表示します。  

## 変更履歴
- 0.2.4 縦横サイズを表示
- 0.2.3 アイコンの変更
- 0.2.2 ファイル移動時は再生状態を引き継ぐように変更、情報表示を変更
- 0.2.1 微修正
- 0.2.0 大幅変更、早送り機能追加
